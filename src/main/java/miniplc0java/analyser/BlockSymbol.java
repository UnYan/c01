    ...
    public String output(){
        ...
        try{
            for (SymbolEntry i : list) {
                result.append(String.format("%02x", i.isConstant ? 1 : 0));
                result.append(String.format("%08x", i.len));
                if(i.isStr){
                    String str = i.string;
                    for (int j=0;j<str.length();j++)
                        result.append(String.format("%02x", (int) str.charAt(j)));
                }
                else
                    result.append("00".repeat(Math.max(0, i.len)));
            }
        }catch (Exception e){
        }
